...
function Signup() {
  ...
  const onSubmit = async (data) => {
    // Only perform admin-specific checks if userType is Admin
    if (data.userType === "Admin" && data.securityKey !== "7429") {
      toast.error("Invalid security key for Admin", { position: "top-right" });
      return;
    }
      ...
      <div className="w-full max-w-md p-8 bg-black/90 rounded-xl shadow-lg relative">
        ...
        >
          ...
          <div className="mb-4 w-full">
            ...
            <select
              {...register("userType")}
              className="w-full px-4 py-3 border border-gray-400 rounded-full  text-white placeholder-gray-400 focus:border-pink-500 focus:outline-none"
              value={userType}
          ...
          {/* Security Key - Show only for Admin */}
          {userType === "Admin" && (
            <div className="mb-4 w-full">
              <label className="block mb-2 text-gray-200 font-semibold">
                Security Key
              ...
              <input
                ...
                {...register("securityKey", {
                  required: userType === "Admin" && "Security key is required",
                })}
              />
              {errors.securityKey && (
                <span className="text-sm text-red-500 block text-center mt-1">
